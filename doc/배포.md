...
3. **코드 서명 및 보안 (macOS)**
   - 본 프로젝트는 애플 개발자 계정을 통한 공식 서명/공증(Notarization) 없이 **"확인되지 않은 개발자(Unidentified Developer)"** 방식으로 배포됩니다.
   - **이점**: 유료 개발자 계정 없이도 배포가 가능하며, "손상됨" 에러를 방지합니다.
   - **실행 방법**: 사용자가 앱을 처음 실행할 때, 앱 아이콘 위에서 `우클릭(또는 Control+클릭) > 열기`를 선택하고 대화상자에서 `열기`를 다시 클릭해야 합니다.
   - **설정**: `package.json`의 `mac.gatekeeperAssess`를 `false`로 설정하여 빌드 시 로컬 검증을 건너跳니다.
